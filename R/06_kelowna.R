...
reservations_and_prices_kl <- 
  daily %>% 
  filter(housing, date >= "2017-06-01", status == "R", city == "Kelowna") %>% 
  group_by(date) %>% 
...
monthly_series_kl <- 
  monthly_series_kl %>% 
  left_join(reservations_forecast_kl, by = "yearmon") %>% 
  left_join(price_forecast_kl, by = "yearmon")
...
reservations_and_prices_kl <- 
  ...
  left_join(select(monthly_series_kl, -res, -price), by = "yearmon") %>%
  group_by(yearmon) %>%
...
res_total_pandemic_pct_kl <-
  {(parse_number(res_total_pandemic_kl) - parse_number(res_total_pandemic_dif_kl)) / 
      parse_number(res_total_pandemic_kl)} %>%
  scales::percent(0.1)

res_total_pandemic_trend_kl <-
  reservations_and_prices_kl |> 
  filter(date >= "2020-03-01") |> 
...
max_price_kl <- scales::dollar(price_difs_kl$max_price, 1)

var_rise_price_kl <-
  daily |> 
  filter(housing, city == "Kelowna", year(date) %in% c(2019, 2021),
...
kelowna_res_graph <- 
  ...
  geom_ribbon(aes(x = date, ymin = res, ymax = res_trend, group = 1),
              data = {reservations_and_prices_kl %>% 
                  mutate(res = slider::slide_dbl(res, mean, .before = 6, .complete = TRUE))
              }, fill = col_palette[3],
              alpha = 0.3) +
...
ggsave("output/kelowna_res_graph.png", plot = kelowna_res_graph, width = 9, 
       height = 6)
...
ggsave("output/kelowna_price_graph.png", plot = kelowna_price_graph, width = 9, 
       height = 6)
