...
reservations_and_prices_km <- 
  daily %>% 
  filter(housing, date >= "2017-06-01", status == "R", city == "Kamloops") %>% 
  group_by(date) %>% 
...
monthly_series_km <- 
  monthly_series_km %>% 
  left_join(reservations_forecast_km, by = "yearmon") %>% 
  left_join(price_forecast_km, by = "yearmon")
...
reservations_and_prices_km <- 
  ...
  left_join(select(monthly_series_km, -res, -price), by = "yearmon") %>%
  group_by(yearmon) %>%
...
res_total_pandemic_pct_km <-
  {(parse_number(res_total_pandemic_km) - parse_number(res_total_pandemic_dif_km)) / 
      parse_number(res_total_pandemic_km)} %>%
  scales::percent(0.1)

res_total_pandemic_trend_km <-
  reservations_and_prices_km |> 
  filter(date >= "2020-03-01") |> 
...
max_price_km <- scales::dollar(price_difs_km$max_price, 1)

var_rise_price_km <-
  daily |> 
  filter(housing, city == "Kamloops", year(date) %in% c(2019, 2021),
...
kamloops_res_graph <- 
  ...
  geom_ribbon(aes(x = date, ymin = res, ymax = res_trend, group = 1),
              data = {reservations_and_prices_km %>% 
                  mutate(res = slider::slide_dbl(res, mean, .before = 6, .complete = TRUE))
              }, fill = col_palette[3],
              alpha = 0.3) +
...
ggsave("output/kamloops_res_graph.png", plot = kamloops_res_graph, width = 9, 
       height = 6)

kamloops_price_graph <- 
  reservations_and_prices_km %>% 
...
ggsave("output/kamloops_price_graph.png", plot = kamloops_price_graph, width = 9, 
       height = 6)
