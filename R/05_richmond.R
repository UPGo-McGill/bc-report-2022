...
reservations_and_prices_rm <- 
  daily %>% 
  filter(housing, date >= "2017-06-01", status == "R", city == "Richmond") %>% 
  group_by(date) %>% 
...
monthly_series_rm <- 
  monthly_series_rm %>% 
  left_join(reservations_forecast_rm, by = "yearmon") %>% 
  left_join(price_forecast_rm, by = "yearmon")
...
reservations_and_prices_rm <- 
  ...
  left_join(select(monthly_series_rm, -res, -price), by = "yearmon") %>%
  group_by(yearmon) %>%
...
res_total_pandemic_pct_rm <-
  {(parse_number(res_total_pandemic_rm) - parse_number(res_total_pandemic_dif_rm)) / 
      parse_number(res_total_pandemic_rm)} %>%
  scales::percent(0.1)

res_total_pandemic_trend_rm <-
  reservations_and_prices_rm |> 
  filter(date >= "2020-03-01") |> 
...
max_price_rm <- scales::dollar(price_difs_rm$max_price, 1)

var_rise_price_rm <-
  daily |> 
  filter(housing, city == "Richmond", year(date) %in% c(2019, 2021),
...
richmond_res_graph <- 
  ...
  geom_ribbon(aes(x = date, ymin = res, ymax = res_trend, group = 1),
              data = {reservations_and_prices_rm %>% 
                  mutate(res = slider::slide_dbl(res, mean, .before = 6, .complete = TRUE))
              }, fill = col_palette[3],
              alpha = 0.3) +
...
ggsave("output/richmond_res_graph.png", plot = richmond_res_graph, width = 9, 
       height = 6)

richmond_price_graph <- 
  reservations_and_prices_rm %>% 
...
ggsave("output/richmond_price_graph.png", plot = richmond_price_graph, width = 9, 
       height = 6)
