...
active_daily_R <- 
  daily |> 
  filter(status %in% c("A", "R"), tier == "CMA") |> 
  count(CSDUID, date) |> 
...
active_daily_K <- 
  daily |> 
  filter(status %in% c("A", "R"), tier == "RES") |> 
  count(CSDUID, date) |> 
  left_join(select(st_drop_geometry(CSD), CSDUID = GeoUID, name, dwellings),
            by = "CSDUID") |> 
...
housing_loss_2023_raw_K <- 
  housing_loss_daily_model_K |> 
  filter(date == "2023-12-31") |> 
  pull(units_trend)

rent_change_2023_table_K <- 
  rent_change_table_raw_K |> 
  filter(year == 5) |> 
  mutate(housing_loss = iv * renters / 100) |> 
  mutate(new_housing_loss = (housing_loss_2023_raw_K - sum(housing_loss)) * 
           (housing_loss / sum(housing_loss))) |> 
  mutate(str_2023 = new_housing_loss / renters * 100 * 
           model$coefficients[["iv"]],
         year_2023 = model$coefficients[["year"]]) |> 
...
qs::qsavem(property_R, active_avg_2021_R, active_avg_2019_R, hosts_avg_2021_R,
           ...
           rent_inc_annual_2021_2023_K, rent_change_2023_table_K,
